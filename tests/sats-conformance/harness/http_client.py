...
class FakeHTTPClient:
    def __init__(
        self,
        *,
        validate_sco_before_send: bool = True,
        force_cleartext: bool = False,
        strip_substrate_headers: bool = False,
    ) -> None:
    ...
    def build_request(
        ...
        path: str = "/",
        sco: SCO | None = None,
        accept_substrate: int | None = None,
        ...
        if sco is not None:
            ...
            headers["Substrate-RTT-Budget"] = (
                f"{sco.rtt_budget_ms}; layer={sco.layer}; enforcement=soft"
            )
            headers["Substrate-Traversal"] = ", ".join(
                str(t.layer) for t in sco.traversal_vector
            )
...
class FakeHTTPServer:
    def __init__(
        self,
        *,
        registered_layer_handlers: dict[int, str] | None = None,
        server_max_sats_layer: int = 8,
        cleartext_allowed: bool = False,
        require_sco_for_high_layers: bool = True,
    ) -> None:
        self._handlers = (
            registered_layer_handlers
            or {i: f"handler_L{i}" for i in range(9)}
        )
    ...
    def handle(self, req: FakeHTTPRequest) -> FakeHTTPResponse:
        ...
        if layer is not None and layer >= 3 and self._require_sco:
            ...
            try:
                ...
                return FakeHTTPResponse(
                    status=560,
                    body=b"Malformed Substrate-Context",
                )
            ...
            if sco_dict.get("version") != 1:
                return FakeHTTPResponse(
                    status=560,
                    body=b"SATS_SCO_INVALID: bad version",
                )

            if not (0 <= sco_layer <= 8):
                return FakeHTTPResponse(
                    status=560,
                    body=b"SATS_SCO_INVALID: layer out of range",
                )

            if ts + ttl < now_ms:
                return FakeHTTPResponse(
                    status=560,
                    body=b"SATS_SCO_INVALID: expired",
                )
            ...
            if fidelity < min_fid:
                return FakeHTTPResponse(
                    status=562,
                    body=b"SATS_FIDELITY_BELOW_MIN",
                )

            if layer is not None and layer > sco_layer:
                return FakeHTTPResponse(
                    status=560,
                    body=b"SATS_SCO_INVALID: layer claim exceeds SCO depth",
                )

        # Accept-Substrate negotiation
        accept_min = req.headers.get("Accept-Substrate")
        if accept_min is not None:
            if self._server_max_sats_layer > int(accept_min):
                return FakeHTTPResponse(
                    status=560,
                    body=b"No representation at requested substrate level",
                )
        ...
        if layer is not None and layer in self._handlers:
            ...
            return FakeHTTPResponse(
                status=560,
                body=b"No handler for substrate layer",
            )
        else:
            self._routed_to_handler = "default"

        resp_headers = {
            "Substrate-Layer": str(layer or 0),
            "Substrate-Fidelity": "1.0",
        }
