...
class OrderingEngine:
    ...
    def enqueue(self, msg: Message) -> int | None:
        # Identify layers with unacked messages
        pending_layers = [
            l for l, q in self._queues.items()
            if any(not m.acked for m in q)
        ]

        if pending_layers:
            max_pending = max(pending_layers)

            # Ordering violation: cannot send a message at a higher layer
    ...
    def drain_ready(self) -> list[Message]:
        ...
        for layer in sorted(self._queues):
            # All lower layers must be fully acked
