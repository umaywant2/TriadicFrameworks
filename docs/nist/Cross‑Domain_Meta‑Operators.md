...
### NIST Triadic Atlas — visual diagram (Mermaid)

```mermaid
flowchart TD

    subgraph R0["R0 — Operator Layer (Institutional Invariants)"]
      R0_MEAS["Measurability\n(invariance, traceability, reproducibility)"]
      R0_CAUSAL["Causal Structure\n(models can link cause ↔ effect)"]
      R0_FAIR["Fairness & Accountability\n(standards, public trust)"]
    end
    ...
    subgraph R3["R3 — Energetic Layer (NIST Outputs)"]
      R3_STD["Standards & Reference\nMaterials"]
      R3_CAL["Calibration Services\n& Traceability Chains"]
      R3_TEST["Conformance & Interop\nTesting"]
      R3_DATA["Reference Data &\nBenchmark Suites"]
      R3_GUIDE["Guidelines, Handbooks,\nBest Practices"]
    end
    ...
    %% R1 → R2 (mission shaping coherence)
    R1_QUANT --> R2_PHYS
    R1_QUANT --> R2_MET
    R1_QUANT --> R2_EM
...
*(You can drop this straight into a Mermaid-enabled doc as the “NIST Triadic Atlas” section.)*
