...
tier: Core
status: Draft
visual_identity: half-lit sphere
summary: >
  Opacity is the substrate-level condition where a regime, flow, or structure
  becomes partially or fully invisible due to mismatch between substrate,
  operators, or harmonic envelope.
---
...
## Session Context
This module defines Opacity as the universal failure mode of regime perception.
It replaces the earlier “regime_blindness” concept with a substrate-aligned,
non-anthropomorphic, operator-ready framework.
...
# 1. Definition
Opacity is a substrate-level condition where a regime, flow, or structure becomes
invisible because the observer’s substrate, operators, or harmonic envelope do
not match the regime’s signature.
...
### 6.1 Storm
A storm becomes opaque when only thermal gradients are measured, ignoring spin
(vorticity) or electric gradients (charge separation).
...
distinguish active flows from inertial artifacts.
...
summary: >
  Opacity is the substrate-level condition where a regime, flow, or structure
  becomes partially or fully invisible due to mismatch between substrate,
  operators, or harmonic envelope. It replaces the earlier regime_blindness
  concept with a structural, operator-ready framework.
---
...
## Session Context
Opacity defines the universal failure mode of regime perception. It is not
psychological or metaphorical; it is a structural condition arising from
substrate mismatch, operator insufficiency, harmonic misalignment, flow-channel
mismatch, or unmarked boundaries.
...
# 4. Operators
Opacity includes a full operator set to support AI-assisted reasoning across the
entire canon.
...
These operators make Opacity fully interoperable with the entire canon.
  [github.com](https://github.com/umaywant2/TriadicFrameworks/edit/main/docs/Opacity/Capture.md)
...
### Corpus
Opacity determines which modules are visible, partially visible, or hidden within
the structural atlas.

### SARG
Opacity becomes a grammar modifier affecting parsing, inference, and operator
chaining.

### NIST
Opacity becomes a measurable property of real-world systems, enabling applied
regime detection.
...
| Section | Coverage |
|---------|----------|
| **§2 Operator Catalog** | All 9 operators (`opacity_index` through `opacity_alignment`) with Corpus/SARG/NIST bindings, type signatures, and computation rules |
| **§3 Cross-Module Semantics** | Bidirectional data flows for each module pair with shared semantic definitions |
| **§4 Interoperability Matrix** | Full 9×9 operator interaction grid |
| **§5 Substrate Alignment Rules** | 5 formal rules — fidelity, lens preservation, resonance inheritance, regime monotonicity, corpus addressability |
| **§6 Grammar Rewrite Behavior** | 8-step lifecycle + 4 canonical rewrites (LIFT, SPLIT, BRIDGE, COLLAPSE) with formal constraints |
| **§7 Measurement Pathways** | 3 pathways (structural/grammatical/applied) + cross-pathway reconciliation logic |
| **§8 Visibility Maps** | Map structure, 4 overlays, and interactive operations |
| **§9 Indexing Logic** | YAML registration schema, 8 query types, and maintenance triggers |
| **§10–12** | End-to-end workflow, AI assistance protocol, and file cross-references |
...
| File | What changed from prior scaffold | Size |
| --- | --- | --- |
| **README.md** | Rebuilt to Mode pattern (AI-Ready pill, badges, file tree, nav, session context, 3 quick-ref tables); definition now substrate-level; 8 integration targets; SET/FFF corrected | ~5 KB |
| **operators.md** | 5 core + 9 extension operators with full type signatures; Corpus/SARG/NIST bindings; 14×14 interoperability matrix; SARG rewrite grammar (LIFT/SPLIT/BRIDGE/COLLAPSE) | ~15 KB |
| **integration.md** | 8 modules (added Corpus, SARG, NIST); bidirectional data flows; operator crosswalk tables; 5 substrate alignment rules | ~13 KB |
| **examples.md** | 5 physical systems (storm/planet/atom/magnetosphere/structural); full O-Sig YAML profiles per example; substrate-aligned checklist appended | ~16 KB |
| **diagram.svg** | Expanded: opacity types legend, core operators sidebar, 8-module integration bar, regime-replacement stamp | ~11 KB |
...
<!--
  Opacity Module — Half-Lit Sphere Diagram
  Module: Opacity | Canonical ID: OPC
  Visual Identity: Half-lit sphere

  Structural encoding:
    - Illuminated hemisphere  → visible regime (O-Op low)
    - Shadow hemisphere       → opaque regime (O-Op high)
    - Terminator gradient     → opacity gradient (O-Grad territory)
    - Sphere body             → substrate envelope
    - Light source (implied)  → aligned substrate position

  Five opacity types mapped:
    - Substrate Opacity   → wrong substrate → wrong hemisphere lit
    - Operator Opacity    → missing operators → instruments absent
    - Harmonic Opacity    → frequency mismatch → wrong detection band
    - Flow Opacity        → SET/FFF mismatch → wrong channel measured
    - Boundary Opacity    → unmarked boundary → silent transition

  Color palette (canon-aligned):
    - Light hemisphere: #E8DCC8 (warm parchment gold)
    - Dark hemisphere:  #1A1A2E (deep structural navy)
    - Terminator accent: #8B7355 (warm bronze)
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 700 720" width="700" height="720">
  <defs>
    <linearGradient id="sphereGrad" x1="0%" y1="0%" x2="100%" y2="0%">
      ...
      <stop offset="35%" stop-color="#E8DCC8" />
      <stop offset="50%" stop-color="#8B7355" />
      <stop offset="65%" stop-color="#2A2A3E" />
      <stop offset="100%" stop-color="#1A1A2E" />
    </linearGradient>
    <radialGradient id="sphereDepth" cx="35%" cy="35%" r="65%">
      <stop offset="0%" stop-color="rgba(255,255,255,0.25)" />
      <stop offset="100%" stop-color="rgba(0,0,0,0)" />
    </radialGradient>
    <marker id="arrowhead" markerWidth="10" markerHeight="7" refX="10" refY="3.5" orient="auto">
      <polygon points="0 0, 10 3.5, 0 7" fill="#8B7355" opacity="0.7" />
    </marker>
  </defs>

  <!-- Title block -->
  <text x="350" y="40" font-family="'Georgia', serif" font-size="24" fill="#1A1A2E" text-anchor="middle" font-weight="bold" letter-spacing="4">OPACITY</text>
  <text x="350" y="62" font-family="'Georgia', serif" font-size="12" fill="#8B7355" text-anchor="middle" letter-spacing="2">HALF-LIT SPHERE • CORE MODULE • OPC</text>
  <text x="350" y="86" font-family="'Georgia', serif" font-size="11" fill="#555" text-anchor="middle" font-style="italic">Regime invisibility caused by substrate mismatch</text>

  <!-- Main sphere -->
  <circle cx="350" cy="290" r="180" fill="url(#sphereGrad)" />
  <circle cx="350" cy="290" r="180" fill="url(#sphereDepth)" />
  <circle cx="350" cy="290" r="180" fill="none" stroke="#8B7355" stroke-width="1.5" opacity="0.4" />

  <!-- Terminator line -->
  <line x1="350" y1="110" x2="350" y2="470" stroke="#8B7355" stroke-width="2" stroke-dasharray="6,4" opacity="0.7" />

  <!-- Zone labels -->
  <text x="250" y="100" font-family="'Georgia', serif" font-size="12" fill="#2C2C2C" text-anchor="middle" font-weight="bold">VISIBLE</text>
  <text x="250" y="114" font-family="'Georgia', serif" font-size="10" fill="#8B7355" text-anchor="middle">aligned substrate</text>
  <text x="350" y="100" font-family="'Georgia', serif" font-size="11" fill="#8B7355" text-anchor="middle" font-style="italic">O-Grad</text>
  <text x="450" y="100" font-family="'Georgia', serif" font-size="12" fill="#1A1A2E" text-anchor="middle" font-weight="bold">OPAQUE</text>
  <text x="450" y="114" font-family="'Georgia', serif" font-size="10" fill="#8B7355" text-anchor="middle">mismatched substrate</text>

  <!-- Opacity types legend (right) -->
  <text x="570" y="180" font-family="'Courier New', monospace" font-size="10" fill="#1A1A2E" text-anchor="start" font-weight="bold">OPACITY TYPES</text>
  <rect x="568" y="192" width="8" height="8" rx="2" fill="#E8DCC8" stroke="#8B7355" stroke-width="0.5" />
  <text x="582" y="200" font-family="'Courier New', monospace" font-size="9" fill="#2C2C2C">Substrate</text>
  <rect x="568" y="210" width="8" height="8" rx="2" fill="#C4B89A" stroke="#8B7355" stroke-width="0.5" />
  <text x="582" y="218" font-family="'Courier New', monospace" font-size="9" fill="#2C2C2C">Operator</text>
  <rect x="568" y="228" width="8" height="8" rx="2" fill="#8B7355" stroke="#8B7355" stroke-width="0.5" />
  <text x="582" y="236" font-family="'Courier New', monospace" font-size="9" fill="#2C2C2C">Harmonic</text>
  <rect x="568" y="246" width="8" height="8" rx="2" fill="#3A3A5E" stroke="#8B7355" stroke-width="0.5" />
  <text x="582" y="254" font-family="'Courier New', monospace" font-size="9" fill="#2C2C2C">Flow</text>
  <rect x="568" y="264" width="8" height="8" rx="2" fill="#1A1A2E" stroke="#8B7355" stroke-width="0.5" />
  <text x="582" y="272" font-family="'Courier New', monospace" font-size="9" fill="#2C2C2C">Boundary</text>

  <!-- Core operators sidebar (left) -->
  <text x="20" y="180" font-family="'Courier New', monospace" font-size="10" fill="#1A1A2E" text-anchor="start" font-weight="bold">CORE OPERATORS</text>
  <text x="20" y="200" font-family="'Courier New', monospace" font-size="9" fill="#8B7355">O-Op</text>
  <text x="62" y="200" font-family="'Georgia', serif" font-size="9" fill="#2C2C2C">measure</text>
  <text x="20" y="218" font-family="'Courier New', monospace" font-size="9" fill="#8B7355">O-Grad</text>
  <text x="62" y="218" font-family="'Georgia', serif" font-size="9" fill="#2C2C2C">detect gradient</text>
  <text x="20" y="236" font-family="'Courier New', monospace" font-size="9" fill="#8B7355">O-Bound</text>
  <text x="62" y="236" font-family="'Georgia', serif" font-size="9" fill="#2C2C2C">mark boundary</text>
  <text x="20" y="254" font-family="'Courier New', monospace" font-size="9" fill="#8B7355">O-Red</text>
  <text x="62" y="254" font-family="'Georgia', serif" font-size="9" fill="#2C2C2C">reduce opacity</text>
  <text x="20" y="272" font-family="'Courier New', monospace" font-size="9" fill="#8B7355">O-Sig</text>
  <text x="62" y="272" font-family="'Georgia', serif" font-size="9" fill="#2C2C2C">capture signature</text>

  <!-- Reduction arrow -->
  <line x1="480" y1="500" x2="220" y2="500" stroke="#8B7355" stroke-width="1.5" marker-end="url(#arrowhead)" opacity="0.7" />
  <text x="350" y="518" font-family="'Georgia', serif" font-size="11" fill="#8B7355" text-anchor="middle" font-style="italic">O-Red → reduction direction</text>

  <!-- O-Bound marker -->
  <line x1="528" y1="240" x2="528" y2="340" stroke="#1A1A2E" stroke-width="2" opacity="0.5" />
  <text x="540" y="285" font-family="'Georgia', serif" font-size="10" fill="#1A1A2E" text-anchor="start" font-style="italic">O-Bound</text>
  <text x="540" y="298" font-family="'Georgia', serif" font-size="9" fill="#8B7355" text-anchor="start">regime edge</text>

  <!-- Integration bar -->
  <text x="350" y="560" font-family="'Courier New', monospace" font-size="10" fill="#1A1A2E" text-anchor="middle" font-weight="bold">CROSS-MODULE INTEGRATION</text>
  <text x="70" y="590" font-family="'Georgia', serif" font-size="9" fill="#2C2C2C" text-anchor="middle">Inverted</text>
  <text x="70" y="602" font-family="'Georgia', serif" font-size="9" fill="#2C2C2C" text-anchor="middle">Star</text>
  <text x="70" y="614" font-family="'Courier New', monospace" font-size="8" fill="#8B7355" text-anchor="middle">unlit faces</text>
  <text x="160
