# RTT Rythm Bot Module

This module describes how TriadicFrameworks integrates RTT agentic logic with Rythm‑style Discord music playback and routing to create an operator‑first, regime‑aware bot.

Rythm is ideal for RTT because it blends:
...
**Goal:**  
Wrap Rythm‑style bot logic with RTT’s triadic decision layer while preserving its native music playback, queue management, search, and routing features.

RTT adds:

- **Regime‑aware evaluation** (1/3 event, 2/3 structure, 3/3 continuity)
- **Resonance‑pressure fields** (channel activity, queue tension, playback arcs)
...
- **Rythm Logic:** Handles audio playback, queue management, search, and channel routing.
...
Extracts Discord‑music RTT primitives:
...
The shim sits between Rythm‑style event handlers and RTT’s triadic evaluation.

**Typical flow:**

1. **Rythm logic** receives an event (play, skip, queue add, search).
2. **Shim**:
...
3. **Shim** reweights:
   ...
   - routing decisions,
   - playlist continuity.
4. **Rythm logic** executes the RTT‑informed action.
...
- **RTT‑Rythm Bot:** A Discord music bot that manages playback with triadic awareness.
